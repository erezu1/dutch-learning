      ...
      {COAT_IDS.map((id) => {
        ...
        return (
          ...
          >
            ...
            <svg viewBox="20 -4 100 100" className="h-[26px] w-[26px]" aria-hidden="true">
...
function faceMark(id: CoatId): string {
  const c = COATS[id]
  // A soft patch is blurred here exactly as it is on her, at the same spread
  // in the same units — a siamese's points have no edge, and drawing them as
  // a flat shape at nine tenths made her dot a cat with a brown panel stuck
  // on it. The one thing her coat does is fade, so the dot has to fade too.
  const patches = (c.patches ?? [])
    .map((p) =>
      p.soft
        ? `<path d="${p.d}" fill="${p.fill}" filter="url(#soft-${id})" opacity=".8"/>`
  ...
  // balance that her dot read as the tuxedo's — the same sentence, "dark cap
  // on cream". This crop leans the other way, onto the side the two coats
  // disagree about: a quarter ginger against a fifth black, so the calico's
  // dot leads with the colour the tuxedo hasn't got. It is as far as the lean
  // can go — any further and the crop cuts her left eye.
  const eye = (cx: number) =>
  ...
  return `<clipPath id="disc-${id}"><circle cx="70" cy="46" r="50"/></clipPath>
    <filter id="soft-${id}" x="-45%" y="-45%" width="190%" height="190%">
